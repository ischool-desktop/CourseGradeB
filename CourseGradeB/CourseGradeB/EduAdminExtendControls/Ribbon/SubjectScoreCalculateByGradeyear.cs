using CourseGradeB.StudentExtendControls.Ribbon;
using FISCA.Data;
...
{
    public partial class SubjectScoreCalculateByGradeyear : BaseForm
    {
        private int _schoolYear, _semester, _gradeYear;
        List<string> _ids;
        ...
        private void BW_Dowork(object sender, DoWorkEventArgs e)
        {
            //取得年級學生ID
            _ids.Clear();
            string sqlcmd = "select student.id from student ";
            sqlcmd += "join class on class.id=student.ref_class_id where student.status=1 and class.grade_year=" + _gradeYear;
            DataTable dt = _Q.Select(sqlcmd);
            foreach (DataRow row in dt.Rows)
            {
                if (!_ids.Contains(row["id"] + ""))
                    _ids.Add(row["id"] + "");
            }
            
            //查詢成績
            string id = string.Join(",", _ids);
            sqlcmd = "select student.name,student.student_number,student.seat_no,class.class_name,teacher.teacher_name,class.grade_year,course.id as course_id,$ischool.course.extend.grade_year as course_grade_year,course.period as period,course.credit as credit,course.subject as subject,$ischool.subject.list.group as group,$ischool.subject.list.type as type,sc_attend.ref_student_id as student_id,sce_take.ref_sc_attend_id as sc_attend_id,sce_take.ref_exam_id as exam_id,xpath_string(sce_take.extension,'//Score') as score ";
            ...
            _logStr.Clear();
            _logStr.Add("學年度:" + _schoolYear + " 學期:" + _semester + " 年級:" + _gradeYear);
            //遞迴每個休課紀錄
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            _gradeYear = int.Parse(cboGradeYear.Text);
